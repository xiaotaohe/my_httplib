...
//启动服务器 Http handler所需数据
typedef struct handler_need 
{
  int sock;
  void* (*_post_work)(int);
}handler_data;
...
void handler_post(int sock,char* path,void* (*post_work)(int));
...
//6.请求的分流
void* handler(void* arg,void* (*post_work)(int)=NULL)
{ 
  ...
  {
    handler_post(sock,path,post_work);
    close(sock);
    return NULL;
  }
  return NULL;
...
//10.获取body
bool get_body(int sock,char body[])
{
  char line[BUF_MAX] = {0};
  const char* length = "Content-Length: ";
  int body_size = 0;
  get_line(sock,line,BUF_MAX-1);
  cout<<"line: "<<line<<endl;
  while(strcmp(line,"\n") != 0)
  {
    if(strncasecmp(line,length,strlen(length)) == 0)
    {
      body_size = atoi(line+strlen(length));
      break;
    }
    get_line(sock,line,BUF_MAX-1);
    cout<<"line: "<<line<<endl;
...
//用来测试的post_work
void* post_work1(int sock)
{
  show_file(sock,"./wwwroot/index.html");
}
//11.处理post方法
void handler_post(int sock,char* path,void* (*post_work)(int))
{
...
void* start(void* arg)
{
  struct handler_need handler_data = *((struct handler_need*)arg);
  handler(&handler_data.sock,handler_data._post_work);
  close(handler_data.sock);
}
...
class Http_Server
{
  public:
    ...
    void get_handler_data(void* (*post_work)(int)=NULL)
    {
      _handler_data._post_work = post_work;
    }

    void Http_Work(void*(*post_work)(int)=NULL)
    {
      //1.采用多线程模型
      cout<<"server start...."<<endl;
      int client_sock;
      //2.获取post的处理函数
      get_handler_data(post_work);
      while(1)
      {
        ...
        //2.通过子线程来服务新连接
        pthread_t ret = 0;
        _handler_data.sock = client_sock;
        pthread_create(&ret,NULL,&start,(void*)&_handler_data);
        pthread_detach(ret);
      }
    }
  private:
